...
> 
> **最近完成：**
> - ✅ v1.1.0 密码加密存储、SSH 密钥认证、完整测试覆盖
> - ✅ v1.2.0 终端输出导出、清屏快捷键、自动重连、快捷键配置、分屏终端、会话保存/恢复
> - ✅ v1.3.0 文件预览、文件编辑、传输暂停/恢复、文件搜索、解压功能
> - ✅ v1.4.0 网络监控、进程管理、历史数据图表、告警功能
> - ✅ v1.5.0 MCP 文件上传/下载工具、连接持久化
> - ✅ v1.6.0 连接导入/导出、最近连接历史、窗口位置记忆、托盘图标
> - ✅ v2.3.3 AI 助手（对话 + 智能体 Agent）、Docker 管理、MCP 增强（Token 鉴权/SSE/日志体系/操作同步终端）、sudo 提权上传、CodeMirror 6 编辑器、MCP 调用链路性能优化
...
| 模块 | 已实现功能 | 完成度 |
|------|-----------|--------|
| **SSH 终端** | 多标签会话、字体主题配置、搜索、右键菜单、系统监控/文件管理/MCP日志面板 | 75% |
| **连接管理** | CRUD、分组、克隆、在线检测、快速导入、测试连接、密钥认证 | 85% |
| **SFTP 文件** | 目录浏览、上传下载、拖拽上传、文件操作、权限修改、压缩/解压、预览编辑、传输暂停恢复、搜索 | 85% |
| **系统监控** | CPU/内存/磁盘/网络监控、进程管理、历史图表、告警、自动刷新 | 85% |
| **MCP 集成** | 16个工具、配置管理、OpenCode/Claude支持、操作日志、npm发布 | 95% |
| **用户体验** | 应用主题、终端主题、字体配置、错误提示、帮助文档、窗口记忆、托盘图标 | 85% |
...
| iter_read_file | 读取远程文件内容 |
| iter_write_file | 写入远程文件内容 |
| iter_upload_file | 上传本地文件到远程 |
| iter_download_file | 从远程下载文件到本地 |
| iter_list_saved_connections | 列出数据库保存的连接 |
| iter_quick_connect | 使用保存的连接快速连接 |
...
| 2 | MCP | SSH 密钥认证 | 1-2天 | MCP 连接仅支持密码 |
