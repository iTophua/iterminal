...
| **连接管理** | CRUD、分组、克隆、在线检测、快速导入、测试连接 | 75% |
...
| 功能 | 优先级 | 说明 | 预估工时 |
|------|--------|------|----------|
| **分屏终端** | 🔴 高 | 无法同时查看多个终端，运维对比操作不便 | 2-3天 |
| **终端输出导出** | 🔴 高 | 无法保存终端内容到文件，排查问题时需手动复制 | 0.5天 |
| **会话保存/恢复** | 🔴 高 | 关闭应用后无法恢复历史会话状态 | 1天 |
| **SSH 密钥认证** | 🔴 高 | 仅支持密码认证，生产环境基本要求 | 1-2天 |
| **自动重连** | 🔴 高 | 网络断开后无自动重连机制 | 1天 |
| **清屏快捷键** | 🟡 中 | 无 Ctrl+L 清屏功能 | 0.5天 |
| **终端链接检测** | 🟡 中 | 无法点击 URL/文件路径进行打开 | 1天 |
| **会话重命名** | 🟢 低 | 标题固定为"会话1/2/3"，不支持自定义 | 0.5天 |
| **标签拖拽排序** | 🟢 低 | 会话标签无法拖拽排序 | 0.5天 |
...
| 功能 | 优先级 | 说明 | 预估工时 |
|------|--------|------|----------|
| **密码加密存储** | 🔴 高 | 当前明文存储在 localStorage，存在安全风险 | 1-2天 |
...
| **书签/收藏目录** | 🟢 低 | 无收藏功能 | 0.5天 |
| **传输限速** | 🟢 低 | 无上传/下载速度限制 | 0.5天 |
...
| **告警功能** | 🟡 中 | 无阈值告警和通知机制 | 1天 |
| **多服务器对比** | 🟢 低 | 只能单服务器查看 | 1天 |
| **服务状态 (systemd)** | 🟢 低 | 无 systemd 服务管理 | 2天 |
...
| 序号 | 模块 | 功能 | 预估工时 | 说明 |
|------|------|------|----------|------|
| 1 | 安全 | 密码加密存储 | 1-2天 | 安全合规风险 |
| 2 | 安全 | 主机密钥验证 | 0.5天 | 中间人攻击风险 |
| 3 | 连接 | SSH 密钥认证 | 1-2天 | 生产环境基本要求 |
| 4 | 终端 | 分屏功能 | 2-3天 | 运维对比操作刚需 |
| 5 | 终端 | 输出导出 | 0.5天 | 排查问题必需 |
| 6 | 终端 | 会话保存/恢复 | 1天 | 用户体验刚需 |
| 7 | 终端 | 自动重连 | 1天 | 稳定性必需 |
| 8 | SFTP | 文件预览 | 2天 | 高频操作 |
| 9 | SFTP | 文件编辑 | 2-3天 | 高频操作 |
| 10 | SFTP | 传输暂停/恢复 | 1-2天 | 大文件传输必需 |
| 11 | 监控 | 网络监控 | 1天 | 运维盲区 |
| 12 | 监控 | 进程管理 | 1-2天 | 运维常用 |
| 13 | MCP | 文件上传/下载工具 | 1天 | AI 无法传输文件 |
| 14 | UX | 快捷键配置 | 1-2天 | 设置入口已存在 |
